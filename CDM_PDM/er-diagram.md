...
### 1. 临时锁座记录表（TEMP_SEAT_LOCKS）不建物理表的说明

在概念设计（E-R 图）中，为了展示完整的业务流程，我们画了 `TEMP_SEAT_LOCKS`（临时锁座记录表）。但在实际建库（MySQL）时，我们**没有**创建这张物理表。

**理由如下：**

- 用户选座、锁座的操作非常频繁，且锁定状态最多只保留 15 分钟（超时自动释放）。如果直接在 MySQL 里频繁进行数据的插入和删除，会产生大量的磁盘读写开销，拖慢数据库。
- 并发抢座时，直接用 MySQL 的行锁来控制，极易导致高并发下的数据库卡死和死锁。

**替代方案：** 我们把临时锁座的业务逻辑**交由 Redis 内存数据库来实现**。利用 Redis 全内存、高性能的特点，在内存中完成 15 分钟锁定与自动释放，让核心的 MySQL 数据库变得更轻量。

### 2. 基于 Redis 的选座与抢票控制

为了应对用户频繁查看座位和并发抢票的场景，系统采用了 **MySQL 负责持久化、Redis 负责抗并发** 的架构：

- **场次座位预热**：管理员创建好放映场次后，系统会自动在 Redis 中生成一个专属的哈希（Hash）结构，把这个影厅的所有座位初始化进 Redis 内存（状态标记为：`0-可选`、`1-已锁定`、`2-已售`）。用户打开选座页面时，**不查询 MySQL，直接读取 Redis 内存**，实现毫秒级的页面加载。
- **内存原子锁座**：用户提交抢座时，系统直接在 Redis 内存中将状态从 `0` 改为 `1`。因为 Redis 是单线程处理命令的，能绝对保证同一个座位在同一物理瞬间只会被一个用户锁定制服。
- **异步/同步落库**：用户在 15 分钟内付款成功后，Redis 里的座位状态变成 `2（已售）`。此时后端才向 MySQL 的 `ORDERS`（订单表）和 `ORDER_SEATS`（订单座位表）真正写入永久数据。

### 3. 基于版本号（Version）的钱包乐观锁控制

用户的虚拟钱包余额（`wallet_balance`）属于核心敏感数据。为了防止用户在手机上狂点、连击支付按钮，或者退款与扣款并发发生时导致余额加减出错（覆盖丢失问题），我们在 `USERS`（用户表）中引入了 `version`（版本号）字段。

**具体落地方式：**

1. **先查版本号**：用户扣款前，Java 后端先查出用户当前的余额和版本号（假设此时 `version = 0`）。

2. **带版本号更新**：真正发起扣款 SQL 时，必须带上刚才查到的版本号：

   SQL

   ```
   UPDATE users SET wallet_balance = wallet_balance - 50, version = version + 1 
   WHERE id = #{userId} AND version = #{currentVersion};
   ```

3. **根据结果判断**：

   - 如果更新成功（影响行数为 1），说明期间没人动过钱包，扣款成功，同时数据库里的 `version` 自动变成 1。
   - 如果更新失败（影响行数为 0），说明在更新的一瞬间，别的高并发线程抢先一步把版本号改了。此时系统直接拦截，拒绝扣款，从而100% 保证了钱包的并发安全。
